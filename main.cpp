...
int main(int argc, char **argv) {
    ...
    std::ostringstream ImgData;  // Image data goes to string
    fileObject>>ImgData.rdbuf();  // File data goes into ImgData
    int len=ImgData.str().length();  // Get ImgData as a string, put the length of ImgData into a var
    for(int i=0; i<len; i++) {
        ...
        case '\n':
              break;
        case 'N':
              cout << "\n";  // if image has N. make a new line
              break;
        default:
